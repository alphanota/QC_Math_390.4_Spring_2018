...
ANSWER: The features are special aesthetic and physical properties of the diamonds that may have an effect on the size. For example, the carat corresponds to the size of the diamond.
***TO-DO
...
for (col in colnames(diamonds)) {
  base = ggplot(diamonds, aes_string(col))
  if(is.factor(diamonds[[col]]) ){
    print ('TRUE')
    print (base + geom_bar())
  }
  else{
    print(base + geom_density())
  }
}
...
for (col in colnames(diamonds)) {
  ...
  if(is.factor(diamonds[[col]]) ){
    print ('TRUE')
    print (base + geom_boxplot(aes(y=price)))
  }
  else{
    print(base + geom_point(aes(y=price)))
  }
}
...
```{r}
ggplot(diamonds) +
  geom_point(aes(x=depth,y=color))
```
...
```{r}
#TO-DO
obj = ggplot(diamonds, aes(x = color, y = clarity, color = price)) +
  geom_point(size=0.01) +
  geom_jitter(width=0.1,height=0.1)+
  labs(x = "Diamond Color", y = "uniformity of cell size") + 
...
Depth alone appears to have a very poor predictor for price. 
