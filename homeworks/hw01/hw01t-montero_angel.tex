...
\author{Angel Montero} %STUDENTS: write your name here
...
$\mathcal{H} = \{ \mathds{1} \vec{w} \cdot \vec{x} > 0 : \vec{w} \in \mathbb{R}^{p+1} \} $
...
2. Calculate $\hat{y}_i = 1 \text{ if } \vec{w} \cdot \vec{x} > 0. \text{ Otherwise it is 0.}$\\
3. Update the weights $\vec{w}_j = \vec{w}_j + (y_i - \hat{y})* \vec{x}j$ \\
4. Repeat steps 2 and 3 for all $i \in \{1,..., n\} $
5. Repeat steps 2 through 4 until a treshold error is reached or until a maximum number of iterations.
